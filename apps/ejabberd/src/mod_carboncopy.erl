...
is_bare_to(Direction, To, PrioRes) ->
    case {Direction, To} of
        ...
        %% unavailable JIDs treated like bare JIDs
        {received, #jid{lresource = LRes}} -> is_unavailable(LRes, PrioRes);
        _ -> false
    end.
...
send_copies(JID, To, Packet, Direction)->
    ...
    PrioRes = ejabberd_sm:get_user_resources(U, S),
    CCResList = get_cc_enabled_resources(U, S),

    Targets = case is_bare_to(Direction, To, PrioRes) of
                  true -> jids_minus_max_priority_resource(U, S, R, CCResList, PrioRes);
